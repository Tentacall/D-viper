...
impl Position {
    ...
    fn next(&self, direction: Direction) -> Position {
        ...
        match direction {
            Direction::BOTTOM => Position { posx: self.posx, posy: self.posy - 1 },
            Direction::LEFT => Position { posx: self.posx + 1, posy: self.posy },
            Direction::RIGHT => Position { posx: self.posx - 1, posy: self.posy },
            Direction::TOP => Position { posx: self.posx, posy: self.posy + 1 }
        }
    }
...
impl Snake {
    ...
    fn extend_front(&mut self) -> Result<(), String> {
        if let Some(curr_head) = self.snake.front_mut(){
            ...
            let p : SnakeBodyPart = SnakeBodyPart::new(curr_head.pos.next(self.direction.opposite()), Part::HEAD);
    ...
    fn extend_back(&mut self) -> () {
        if let Some(curr_tail) = self.snake.back_mut() {
            ...
            let p : SnakeBodyPart = SnakeBodyPart::new(curr_tail.pos.next(self.direction), Part::HEAD);
